...
// 3 args
static inline i64 syscall3(u64 num, u64 arg1, u64 arg2, u64 arg3) {
  int64_t ret;
  asm volatile("int $0x80"
               : "=a"(ret)
...
static inline i64 exec(char *file_name) {
  return syscall1(SYS_EXEC, (u64)file_name);
}

static inline i64 mbox_create(u64 id) {
  return syscall1(SYS_MAILBOX_CREATE, id);
}

static inline i64 mbox_send(u64 id, char *data, u64 data_len) {
...
static inline i32 shm_create(u64 size, bool public) {
  return syscall2(SYS_SHM_CREATE, size, public);
}

static inline u64 shm_map(i32 region_id, u64 *out_size) {
  return syscall2(SYS_SHM_MAP, region_id, (u64)out_size);
}
